...
\newcommand{\SimpleGravityPendulum}[4]
    ...
    % !!!   No terminating ';' when calling this command   !!!
{
    \begin{scope}[xshift=#4]
        ...
        % draw part of the circle trajectory of the mass
        \draw[dashed] (FM) ++(-\ARC-90 : \ROD) arc (-\ARC-90 : \ARC-90 : \ROD);

        % draw coordinate system
        \draw[axis] (O) +( -3.10,  0.00) -- +( +3.10, 0.00)                node[below right] {$x$};
        \draw[axis] (O) +(  0.00, -0.25) -- +(  0.00, \ROD + 2*\THICKNESS) node[above right] {$y$};
    	...
    	% draw mass
        \path (MC) node[mass=\MASS,circle] (M) {$m$};

        % angle between rod and vertical y-axis
        \draw[dotted,->,>=stealth] (FM) ++( -90 : 1.5 ) arc ( -90 : \ANGLE-90 : 1.5 );
        \path (FM)++( {0.5*\ANGLE-90} : 1.5 ) node[below=-5pt,fill=white] {$\theta$};

		% indicate rod's length = radius of circular trajectory of the mass
		\draw[latex-latex,dashed,gray] ( $(FM)$ ) -- node[fill=white] {$l$} ( -\ANGLE-90 : \ROD );

        % \draw[dashed] (FM)  +(0.1,0)     --  +(0.9,0);
        % \draw[dashed] (MC) +(0.1,0)     --  +(0.9,0);
		% \draw[latex-latex,dashed] ( $(FM) + (0.5, 0.0)$ ) -- node[fill=white] {$l$} ( $(MC) + (0.5, 0.0)$ );
