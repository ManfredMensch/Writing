    ...
    % !!!   No terminating ';' when calling this command   !!!
{
    \begin{scope}[xshift=#4,yshift=#5]
        ...
        % coordinate system
        \draw[axis] (O) +( -3.10,  0.00) -- +( +3.10, 0.00)
            node[below right] {$x$};
        \draw[axis] (O) +(  0.00, -0.25) -- +(  0.00, \ROD + 2*\THICKNESS)
            node[above right] {$y$};
    ...
    % !!!   No terminating ';' when calling this command   !!!
{
    \begin{scope}[xshift=#4,yshift=#5]
        ...
        % gravitational force
        \draw[force] (MC) -- ++( 0.00, -0.20 * \G)
            node[above left=2pt] {${{\vec{F}}\!_g}$};
        % radial component of gravitational force
        \draw[force,dashed] (MC) -- ++( \ANGLE-90 : {0.20 * \G * cos(\ANGLE)} )
            node[above right=1pt] {${\vec{F}\!_{g,r}}$};
        % tangential component of gravitational force
        \draw[force,dashed] (MC) + ( \ANGLE-90 : {0.20 *\G * cos(\ANGLE)} ) -- ( $(MC) + ( 0.00, -0.20 * \G)$ )
            node[below right=2pt] {${\vec{F}\!_{g,t}}$};

        % angle between vertical and radial component of gravitational force
        \draw[dotted,->,>=stealth] (MC) ++( -90 : 1.5 ) arc ( -90 : \ANGLE-90 : 1.5 );
        \path (MC)++( {0.5*\ANGLE-90} : 1.5 ) node[below=-5pt,fill=white] {$\theta$};
